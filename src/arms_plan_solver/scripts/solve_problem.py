...
def main():
    ...
    # **Merge all final plans**
    final_plan_path = f"{base_output_dir_str}/mergedplan.txt"
    # Class_and_functions.merge_for_multi_path(mergedplans, final_plan_path, domain_file, Path(problem_file))
...
#                 # Compute the **earliest** arrival time for each robot at the next site
#                 travel_times = {}
                
#                 for robot in robots:
#                     travel_time = Class_and_functions.distance(robot_state[robot]["loc"], site_obj.center) / 1.5  # Assuming air travel
#                     travel_times[robot] = robot_state[robot]["time"] + travel_time  # Start from robot's known last time

#                 predecessor_latest_times = {}
#                 predecessor_latest_robot = {}
#                 for p in predecessors:
#                     latest_robot = max(robots, key=lambda r: stn.nodes[p]["latest_finish"] if r in robot_state else float('-inf'))
#                     latest_time = stn.nodes[p]["latest_finish"]
#                     travel_time = Class_and_functions.distance(robot_state[latest_robot]["loc"], site_obj.center) / 1.5

#                     predecessor_latest_times[p] = latest_time + travel_time
#                     predecessor_latest_robot[p] = latest_robot

#                 # **Step 2: Compare latest predecessor arrival time against other predecessors**
                
#                 for p1 in predecessors:
#                     for p2 in predecessors:
#                         if p1 == p2 or sync_created:
#                             continue  # Skip self-comparison

#                         latest_robot_p1 = predecessor_latest_times[p1]  # Latest arrival time from predecessor p1
#                         # print([travel_times[r] for r in robots if r in robot_state], [r for r in robots])
#                         earliest_robot_p2 = min(travel_times[r] for r in robots if r in robot_state)  # Earliest arrival from p2

#                         if earliest_robot_p2 < latest_robot_p1:
                            
#                             # **Create a synchronization "goto" path**
#                             sync_path_id = f"sync_{path_idx}"
#                             sync_robot = min(travel_times, key=travel_times.get)  # Robot arriving early
#                             print(f"⚠️ Robot {sync_robot} from Path {p2} is faster than {predecessor_latest_robot[p1]} from Path {p1}. Creating a 'goto' sync step.")

#                             stn.add_node(sync_path_id, sites=[path[0]], robots=[sync_robot], earliest_start=0, latest_finish=float('inf'), executed=False)
#                             # Adjust STN links with correct time gap
#                             min_time_gap = earliest_robot_p2 - latest_robot_p1
#                             stn.remove_edge(p2, path_idx)
#                             stn.add_edge(p2, sync_path_id, min_time_gap=stn.nodes[p2]["latest_finish"])  # Ensure non-negative time gap
#                             sync_time_gap = max(0, stn.nodes[path_idx]["earliest_start"])
#                             stn.add_edge(sync_path_id, path_idx, min_time_gap=sync_time_gap)
