...
class Poi():
    ...
    def set_flags_from_type(self):
        self.isswitchable = self.typepoi == "transition"
        self.isrelay = self.typepoi == "transition"
        self.isgoal = self.typepoi == "sample"  # or "pollution" if that's your actual goal type
    ...
    for s in mission.sites:
        ...
        for p in s.poi:
            ...
            if p.typepoi == "transition":
                ...
                if getattr(p, "isswitchable", True):
                    initstatelines.append('(isswitchable {0})'.format(p.name))
                if getattr(p, "isrelay", True):
                    initstatelines.append(f"(isrelay {p.name})")
        ...
        # Modify the goal section
            ...
            if goaltype == "goto":      
                ...
                for p in next_site.poi:
                    if p.typepoi == "sample":
                        if getattr(p, "isgoal", True):
                            new_lines.append('(sampled {0})'.format(p.name))
            new_lines.append(")\n")  # Close the `and` block
            new_lines.append(")\n")  # Close `goal`
            new_lines.append(")\n") # Close `define`
...
# def parsedataclassfrompddl(file):
#     """ Create data class from PDDL problem script """
#     mission = Mission(sites=[],robots=[])
#     with open(file, 'r') as f:
#             contents = f.readlines()
...
def recover_mission_from_json(json_file: Path) -> Mission:
    ...
    for point in data["points"]:
        ...
        poi_obj.set_flags_from_type()
        points[point["id"]] = poi_obj
...
def action_shape(action_name):
    ...
    """
    shapes = {
        "navigation_water": "o",  # Circle for navigation actions
        "navigation_air": "o",  # Circle for navigation actions
        "switch_airwater": "s",      # Square for switch actions
        "switch_waterair": "s",      # Square for switch actions
        "translate_data": "d",   # Diamond for translate actions
        "sample": "v",      # Triangle for assess actions
        "takeoff": "o",     # Circle for takeoff
        "change_site": "o", # Circle for change_site
        "observe": "^",     # Triangle for observe
        "observe_2r": "^",     # Triangle for observe
    }
...
def draw_STN(STN, layout='dot', impact_colors={}):
    ...
    Edges between clusters represent temporal dependencies with annotated gaps.

    Args:
        STN: A networkx.DiGraph with nodes labeled by path names and annotated with start/finish times.
        layout: Graphviz layout engine (e.g., 'dot', 'neato').
        impact_colors: Optional dict mapping path names to cluster fill colors (e.g., {"0": "lightcoral"}).
    """
...
def regular_polygon_positions(sites):
    """
    Returns positions for each site arranged in a regular polygon layout.
    
    Input:
        sites: list of Site objects (with .name attributes)

    Output:
        positions: dict {site_name: (x, y)}
    """
    N = len(sites)
    radius = N * 10
    theta = 2 * np.pi / N
    ...
    for i, site in enumerate(sites):
        name = site.name.replace("site", "") if site.name != "base" else "base"
        positions[name] = (round(radius * np.cos(i * theta), 2), round(radius * np.sin(i * theta), 2))
    return positions



def plot_topological_multigraph(robassign, site_list, color_map, site_color_map):
    import matplotlib.pyplot as plt
    import networkx as nx
    ...
    # Get robot-specific colormap
    robots = list(robassign.keys())
    cmaprob = plt.cm.get_cmap("Set2", len(robots))
    colorsrob = [cmaprob(i) for i in range(len(robots))]

    for idx, (robot, path) in enumerate(robassign.items()):
...
def plot_cluster_paths(team_paths, site_positions, site_color_map, team_colors, team_linestyles):
    ...
    plt.title("Repartition of clusters and robots paths")
    plt.legend()
    plt.tight_layout()
    plt.xlim(-300, 1000)
    plt.ylim(-1000, 1000)
    plt.savefig("/tmp/clusterpath.pdf", format="pdf", bbox_inches="tight")
    plt.show()



def plot_site_visits_over_time(visit_data, site_color_map, waiting_data=None):
    import matplotlib.pyplot as plt
    ...
    for i, (robot, visits) in enumerate(visit_data.items()):
        y = i * (height + 5)
        for site, start, end in visits:
            color = site_color_map.get(site, "grey")
            ax.broken_barh([(start, end - start)], (y, height), facecolors=color)
        ylabels.append(robot)
        yticks.append(y + height / 2)

    if waiting_data:
        for robot, wait_periods in waiting_data.items():
            y = list(visit_data.keys()).index(robot) * (height + 5)
...
def extract_inputs_from_mission(mission):
    """
    Extract:
    - allocations: {robot_name: [site0, site1, ...]}
    - site_positions: {site_name: (x, y)}
    - color_map: {robot_name: color}
    """
    import random
    ...
    site_positions = {}
    color_list = [
        "mediumseagreen", "coral", "slateblue", "olive", "yellow",
        "sandybrown", "gray", "tomato", "lightgreen", "skyblue", "plum"
    ]

    # Add base
    base_site = next(s for s in mission.sites if s.name == "base")
    site_positions["base"] = (base_site.center[0], base_site.center[1])

    # Add site positions
    for site in mission.sites:
        if site.name != "base":
            site_positions[site.name.replace("site", "")] = (site.center[0], site.center[1])

    # Allocations from robot history
    for i, robot in enumerate(mission.robots):
        allocations[robot.name] = ["base"] + [s.replace("site", "") for s in robot.histo]
    
    # Color map
    color_map = {robot.name: color_list[i % len(color_list)] for i, robot in enumerate(mission.robots)}

    return allocations, site_positions, color_map
