...
class Observe2rAction : public plansys2::ActionExecutorClient
{
public:
  Observe2rAction()
  : plansys2::ActionExecutorClient("observe_site_2r", 1s), action_in_progress_(false)
  {
    ...
    this->subscription_ = this->create_subscription<plansys2_msgs::msg::ActionExecutionInfo>(
        result_topic, 10, std::bind(&Observe2rAction::result_callback, this, std::placeholders::_1));
  }
...
private:
  void do_work() override
  {
    if (!action_in_progress_) {
      ...
      RCLCPP_INFO(this->get_logger(), "Sending Observe 2r request with action ID: %s and robot ID: %s", current_action_id_.c_str(), robot_id_.c_str());
    }
  }
  ...
  void result_callback(const plansys2_msgs::msg::ActionExecutionInfo::SharedPtr msg)
  {
    if (msg->action == current_action_id_) {
      ...
      if (status == plansys2_msgs::msg::ActionExecutionInfo::FAILED) {
        finish(false, 0.0, "Observe 2r failed for" + msgstatus);
        action_in_progress_ = false;
        } else if (status == plansys2_msgs::msg::ActionExecutionInfo::SUCCEEDED){
        finish(true, 1.0, "Observe 2r completed");
        action_in_progress_ = false;
        } else if (status == plansys2_msgs::msg::ActionExecutionInfo::EXECUTING){
          if (msgstatus[0] == '1'){
          // The action is delayed but we continue
          send_feedback(progress, "Observe 2r running based on simulation feedback with a DELAY");
          } else {
          send_feedback(progress, "Observe 2r running based on simulation feedback");
          }
        }
    }
  }
  ...
  CallbackReturn on_deactivate(const rclcpp_lifecycle::State & state) override
  {
    if (action_in_progress_) {
      ...
      publisher_->publish(msg);
      action_in_progress_ = false;
      RCLCPP_INFO(this->get_logger(), "Action observe 2r with ID: %s and robot ID: %s cancelled", current_action_id_.c_str(), robot_id_.c_str());
    }
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<Observe2rAction>();

  node->set_parameter(rclcpp::Parameter("action_name", "observe_site_2r"));
