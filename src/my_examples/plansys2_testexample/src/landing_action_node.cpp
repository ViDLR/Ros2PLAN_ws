...
LandingActionNode::LandingActionNode(const std::string &robot_name, const std::string &team_name)
: plansys2::ActionExecutorClient(
      "landing_action_node_" + robot_name,  // Node name
      team_name,                                // Namespace
      1s),                                      // Execution rate
...
CallbackReturnT LandingActionNode::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  subscription_ = this->create_subscription<plansys2_msgs::msg::ActionExecutionInfo>(
      result_topic, 10, std::bind(&LandingActionNode::result_callback, this, std::placeholders::_1));

  return CallbackReturnT::SUCCESS;
}

void LandingActionNode::do_work()
{
  if (!action_in_progress_) {
    ...
    RCLCPP_INFO(this->get_logger(), "Sending landing request with action ID: %s and robot ID: %s", current_action_id_.c_str(), robot_name_.c_str());
  }
}

void LandingActionNode::result_callback(const plansys2_msgs::msg::ActionExecutionInfo::SharedPtr msg)
{
  if (msg->action_full_name == current_action_id_) {
    ...
    if (status == plansys2_msgs::msg::ActionExecutionInfo::FAILED) {
      finish(false, 0.0, "Landing failed for" + msgstatus);
      action_in_progress_ = false;
      ...
      if (msgstatus[0] == '1') {
        ...
        send_feedback(progress, "Landing running based on simulation feedback with a DELAY");
      } else {
        send_feedback(progress, "Landing running based on simulation feedback");
      }
    }
  }
}

CallbackReturnT LandingActionNode::on_deactivate(const rclcpp_lifecycle::State & state)
{
  // Call base class on_configure
  ...
  if (action_in_progress_) {
    ...
    publisher_->publish(msg);
    action_in_progress_ = false;
    RCLCPP_INFO(this->get_logger(), "Action Landing with ID: %s and robot ID: %s cancelled", current_action_id_.c_str(), robot_name_.c_str());
  }
