...
NavigationAirActionNode::NavigationAirActionNode(const std::string &robot_name, const std::string &team_name)
: plansys2::ActionExecutorClient(
      "navigation_air_action_node_" + robot_name,  // Node name
      team_name,                                // Namespace
      1s),                                      // Execution rate
...
CallbackReturnT NavigationAirActionNode::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  subscription_ = this->create_subscription<plansys2_msgs::msg::ActionExecutionInfo>(
      result_topic, 10, std::bind(&NavigationAirActionNode::result_callback, this, std::placeholders::_1));
...
void NavigationAirActionNode::do_work()
{
  if (!action_in_progress_) {
    ...
    RCLCPP_INFO(this->get_logger(), "Sending aerial navigation request with action ID: %s and robot ID: %s", current_action_id_.c_str(), robot_name_.c_str());
  }
}

void NavigationAirActionNode::result_callback(const plansys2_msgs::msg::ActionExecutionInfo::SharedPtr msg)
{
  if (msg->action == current_action_id_) {
    ...
    if (status == plansys2_msgs::msg::ActionExecutionInfo::FAILED) {
      finish(false, 0.0, "Aerial navigation failed for" + msgstatus);
      action_in_progress_ = false;
    } else if (status == plansys2_msgs::msg::ActionExecutionInfo::SUCCEEDED) {
      finish(true, 1.0, "Aerial navigation completed");
      action_in_progress_ = false;
    } else if (status == plansys2_msgs::msg::ActionExecutionInfo::EXECUTING) {
      if (msgstatus[0] == '1') {
        // The action is delayed but we continue
        send_feedback(progress, "Aerial navigation running based on simulation feedback with a DELAY");
      } else {
        send_feedback(progress, "Aerial navigation running based on simulation feedback");
      }
    }
  }
}

CallbackReturnT NavigationAirActionNode::on_deactivate(const rclcpp_lifecycle::State & state)
{
  // Call base class on_configure
  ...
  if (action_in_progress_) {
    ...
    publisher_->publish(msg);
    action_in_progress_ = false;
    RCLCPP_INFO(this->get_logger(), "Action navigation_air with ID: %s and robot ID: %s cancelled", current_action_id_.c_str(), robot_name_.c_str());
  }
