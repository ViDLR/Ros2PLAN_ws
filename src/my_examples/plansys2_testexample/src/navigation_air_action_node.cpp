...
class NavigationAirAction : public plansys2::ActionExecutorClient
{
public:
  NavigationAirAction()
  : plansys2::ActionExecutorClient("navigation_air", 1s), action_in_progress_(false)
  {
    ...
    this->subscription_ = this->create_subscription<plansys2_msgs::msg::ActionExecutionInfo>(
        result_topic, 10, std::bind(&NavigationAirAction::result_callback, this, std::placeholders::_1));
  }
...
private:
  void do_work() override
  {
    if (!action_in_progress_) {
      ...
      RCLCPP_INFO(this->get_logger(), "Sending Navigation Air request with action ID: %s and robot ID: %s", current_action_id_.c_str(), robot_id_.c_str());
    }
  }
  ...
  void result_callback(const plansys2_msgs::msg::ActionExecutionInfo::SharedPtr msg)
  {
    if (msg->action == current_action_id_) {
      ...
      if (status == plansys2_msgs::msg::ActionExecutionInfo::FAILED) {
        finish(false, 0.0, "Navigation Air failed for" + msgstatus);
        action_in_progress_ = false;
        } else if (status == plansys2_msgs::msg::ActionExecutionInfo::SUCCEEDED){
        finish(true, 1.0, "Navigation Air completed");
        action_in_progress_ = false;
        } else if (status == plansys2_msgs::msg::ActionExecutionInfo::EXECUTING){
          if (msgstatus[0] == '1'){
          // The action is delayed but we continue
          send_feedback(progress, "Navigation Air running based on simulation feedback with a DELAY");
          } else {
          send_feedback(progress, "Navigation Air running based on simulation feedback");
          }
        }
    }
  }
  ...
  CallbackReturn on_deactivate(const rclcpp_lifecycle::State & state) override
  {
    if (action_in_progress_) {
      ...
      publisher_->publish(msg);
      action_in_progress_ = false;
      RCLCPP_INFO(this->get_logger(), "Action Navigation Air with ID: %s and robot ID: %s cancelled", current_action_id_.c_str(), robot_id_.c_str());
    }
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<NavigationAirAction>();

  node->set_parameter(rclcpp::Parameter("action_name", "navigation_air"));
