...
class SwitchWaterAirAction : public plansys2::ActionExecutorClient
{
public:
  SwitchWaterAirAction()
  : plansys2::ActionExecutorClient("switch_waterair", 1s), action_in_progress_(false)
  {
    ...
    this->subscription_ = this->create_subscription<plansys2_msgs::msg::ActionExecutionInfo>(
        result_topic, 10, std::bind(&SwitchWaterAirAction::result_callback, this, std::placeholders::_1));
  }
...
private:
  void do_work() override
  {
    if (!action_in_progress_) {
      ...
      RCLCPP_INFO(this->get_logger(), "Sending Water to Air switch request with action ID: %s and robot ID: %s", current_action_id_.c_str(), robot_id_.c_str());
    }
  }
  ...
  void result_callback(const plansys2_msgs::msg::ActionExecutionInfo::SharedPtr msg)
  {
    if (msg->action == current_action_id_) {
      ...
      if (status == plansys2_msgs::msg::ActionExecutionInfo::FAILED) {
        finish(false, 0.0, "Switch Water to Air failed for" + msgstatus);
        action_in_progress_ = false;
        } else if (status == plansys2_msgs::msg::ActionExecutionInfo::SUCCEEDED){
        finish(true, 1.0, "Switch Water to Air completed");
        action_in_progress_ = false;
        } else if (status == plansys2_msgs::msg::ActionExecutionInfo::EXECUTING){
          if (msgstatus[0] == '1'){
          // The action is delayed but we continue
          send_feedback(progress, "Switch Water to Air running based on simulation feedback with a DELAY");
          } else {
          send_feedback(progress, "Switch Water to Air running based on simulation feedback");
          }
        }
    }
  }
  ...
  CallbackReturn on_deactivate(const rclcpp_lifecycle::State & state) override
  {
    if (action_in_progress_) {
      ...
      publisher_->publish(msg);
      action_in_progress_ = false;
      RCLCPP_INFO(this->get_logger(), "Action Switch Water to Air with ID: %s and robot ID: %s cancelled", current_action_id_.c_str(), robot_id_.c_str());
    }
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<SwitchWaterAirAction>();

  node->set_parameter(rclcpp::Parameter("action_name", "switch_waterair"));
