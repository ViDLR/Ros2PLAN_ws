...
SampleActionNode::SampleActionNode(const std::string &robot_name, const std::string &team_name)
: plansys2::ActionExecutorClient(
      "sample_action_node_" + robot_name,  // Node name
      team_name,                                // Namespace
      1s),                                      // Execution rate
...
CallbackReturnT SampleActionNode::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  subscription_ = this->create_subscription<plansys2_msgs::msg::ActionExecutionInfo>(
      result_topic, 10, std::bind(&SampleActionNode::result_callback, this, std::placeholders::_1));

  return CallbackReturnT::SUCCESS;
}

void SampleActionNode::do_work()
{
  if (!action_in_progress_) {
    ...
    RCLCPP_INFO(this->get_logger(), "Sending sample request with action ID: %s and robot ID: %s", current_action_id_.c_str(), robot_name_.c_str());
  }
}

void SampleActionNode::result_callback(const plansys2_msgs::msg::ActionExecutionInfo::SharedPtr msg)
{
  if (msg->action_full_name == current_action_id_) {
    ...
    if (status == plansys2_msgs::msg::ActionExecutionInfo::FAILED) {
      finish(false, 0.0, "Sampling failed for" + msgstatus);
      action_in_progress_ = false;
    } else if (status == plansys2_msgs::msg::ActionExecutionInfo::SUCCEEDED) {
      finish(true, 1.0, "Sampling completed");
      action_in_progress_ = false;
    } else if (status == plansys2_msgs::msg::ActionExecutionInfo::EXECUTING) {
      if (msgstatus[0] == '1') {
        // The action is delayed but we continue
        send_feedback(progress, "Sampling running based on simulation feedback with a DELAY");
      } else {
        send_feedback(progress, "Sampling running based on simulation feedback");
      }
    }
  }
}

CallbackReturnT SampleActionNode::on_deactivate(const rclcpp_lifecycle::State & state)
{
  // Call base class on_configure
  ...
  if (action_in_progress_) {
    ...
    publisher_->publish(msg);
    action_in_progress_ = false;
    RCLCPP_INFO(this->get_logger(), "Action Sample with ID: %s and robot ID: %s cancelled", current_action_id_.c_str(), robot_name_.c_str());
  }
